...
solo_outcome <- fread("data/raw/solo_outcomes.csv") 
solo_outcome_br <- solo_outcome %>% filter(!is.na(start)) 

# focus on important data

solo_outcome_br <- solo_outcome_br %>% dplyr::select(nestid, outcome_ka, cr_confirm) 
...
# confirmed criteria
solo_outcome_br[cr_confirm == 2]
solo_outcome_br[3,3] <- 1

solo_outcome_br$outcome_ka <- factor(solo_outcome_br$outcome_ka )

# make outcome a factor variable

solo_outcome_br$outcome_ka <- factor(solo_outcome_br$outcome_ka, levels = c(0,1)) 
solo_outcome_br$cr_confirm <- factor(solo_outcome_br$cr_confirm, levels = c(0,1))
...
initial_solo_obs <- fread(
  "data/raw/solonest_initial_locations.csv"
) 
...
# factorize pebble distribution variable
...
raw_solo <- fread("data/raw/solonest_obs_data_entry_2122.csv")
...
ka_outcome <- fread("data/raw/ka_nest_outcomes21.csv")
...
initial_ka_obs <- fread("data/raw/ka_resight21.csv")

# create vector of successful breeder bandnumbers

nests <- ka_outcome_br %>% filter(outcome_ka == 1)
nests <- c(nests$nest)
...
a_designs <- a_designs %>% dplyr::select(c("nestid", "outcome", "outcome_ka"))
