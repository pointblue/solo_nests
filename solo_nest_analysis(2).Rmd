...
## Model predictions
```{r H2 Predictions}
newdata.h2 <- with(solo_rs, data.frame(pebble_distrib_nearby = factor(rep(0:2, each = 33)), prop_neighbs = rep(seq(from = 0, to = 0.8, by = 0.025), times = 3)))
newdata3 <- cbind(newdata.h2, predict(h2.mod2, newdata = newdata.h2, type = "link", se = TRUE))
...
ggplot(data = newdata3, aes(x = prop_neighbs, y = PredictedProb)) +
  geom_ribbon(aes(ymin = LL, ymax = UL, fill = pebble_distrib_nearby), alpha = 0.2) +
  geom_line(aes(color = pebble_distrib_nearby))
